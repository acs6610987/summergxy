/*
 * Title                   : PhotoPro Gallery Module
 * Version                 : 2.1
 * Copyright               : 2013-2015 CodEasily.com
 * Website                 : http://www.codeasily.com
 */
if(typeof jQuery.fn.gmPhotoPro == 'undefined'){
	function gmPhotoPro(ID){
		return window['GmediaGallery_' + ID];
	}

	(function($, window, document, undefined){
		$.fn.gmPhotoPro = function(method){
			var Container = this,
				ID = '',
				flashVerion = '11',
				Content,
				...
				hex = {
					...
					'property15': '7485c2',
					'property16': 'e0e0e0',
					'buttonsBgColor': '00000',
					'linkColor': 'ffffff',
					'backButtonColorBg': '00000',
				...
				methods = {
					...
					initGallery: function(){// Init the Gallery
						var parameters = {
							...
							attributes = {
								styleclass: 'gmPhotoPro_Flash',
								id: 'gmPhotoPro_ID' + ID + '_Flash'
							};
						if(opt.postID){
						...
						swfobject.embedSWF(opt.moduleUrl + '/gallery.swf', 'gmPhotoPro_ID' + ID + '_Container', '100%', '100%', flashVerion, opt.pluginUrl + '/inc/expressInstall.swf', flashvars, parameters, attributes, methods.callbackFn);
					...
					callbackFn: function(e){// e = {(bool) success, (string) id, (reference to the active HTML object element) ref}
						...
						if(e.success){
							var swfHover = e.ref;
							$('#gmPhotoPro_ID' + ID).on("mouseenter", e.ref,function(){
							...
							$('#gmPhotoPro_ID' + ID + '_Flash', Container).css({'width':size[0], 'height': size[1]});
					...
					noFlash: function(){
						...
						$('#gmPhotoPro_ID'+ID+'_Container', Container).html($('#flashmodule_alternative_'+ID, Container).html() + gmlove);
					...
					alternative: function(){
						...
						var gmBodyScrollTop, gmBodyScrollLeft,
							...
							options = {
								allowUserZoom:false,
								autoStartSlideshow:opt.slideshow,
							...
							instance.addEventHandler('PhotoSwipeOnDisplayImage', function(e){
								...
								if(!$(curel.refObj).hasClass('viewed')){
									...
									if(opt.ajaxurl){
										...
										$.post(opt.ajaxurl, { action: 'gmedia_module_interaction', hit: curid }, function(r){
											console.log(r);
											if(r.views){
