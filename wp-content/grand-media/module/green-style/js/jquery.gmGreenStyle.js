/*
 * Title                   : GreenStyle Gallery Module
 * Version                 : 4.8
 * Copyright               : 2013-2015 CodEasily.com
 * Website                 : http://www.codeasily.com
 */
if(typeof jQuery.fn.gmGreenStyle == 'undefined'){
	function gmGreenStyle(ID){
		return window['GmediaGallery_' + ID];
	}

	(function($, window, document, undefined){
		$.fn.gmGreenStyle = function(method){
			var Container = this,
				ID = '',
				flashVerion = '11',
				Content,
				...
				methods = {
					...
					initGallery: function(){// Init the Gallery
						var parameters = {
							...
							attributes = {
								styleclass: 'gmGreenStyle_Flash',
								id: 'gmGreenStyle_ID' + ID + '_Flash'
							};
						if(opt.postID){
						...
						swfobject.embedSWF(opt.moduleUrl + '/gallery.swf', 'gmGreenStyle_ID' + ID + '_Container', '100%', '100%', flashVerion, opt.pluginUrl + '/inc/expressInstall.swf', flashvars, parameters, attributes, methods.callbackFn);
					...
					callbackFn: function(e){// e = {(bool) success, (string) id, (reference to the active HTML object element) ref}
						...
						if(e.success){
							var swfHover = e.ref;
							$('#gmGreenStyle_ID' + ID).on("mouseenter", e.ref,function(){
							...
							var bars_height = (Content.length > 1)? 50 : 20;
							var size = prototypes.responsive_size(bars_height);
							$('#gmGreenStyle_ID' + ID + '_Flash', Container).css({'width':size[0], 'height': size[1]});
					...
					noFlash: function(){
						...
						$('#gmGreenStyle_ID'+ID+'_Container', Container).html($('#flashmodule_alternative_'+ID, Container).html() + gmlove);
