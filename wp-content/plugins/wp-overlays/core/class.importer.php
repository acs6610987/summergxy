...
if ( ! class_exists( 'FlipperCode_Export_Import' ) ) {
	...
	class FlipperCode_Export_Import {
		...
		*/
		...
		function export($action, $asFilename) {
			...
			if ( 'csv' == $action ) {
				...
				if ( ! empty( $this->data ) ) {
					$wop_tab = "\t";
					$wop_br = "\n";
					$wop_xml_writter  = '<?xml version="1.0" encoding="UTF-8"?>'.$wop_br;
					$wop_xml_writter .= '<items>'.$wop_br;

					foreach ( $this->data as $key => $result ) {
						$wop_xml_writter .= $wop_tab.'<item>'.$wop_br;
						foreach ( $result as $node_key => $node_value ) {
							$wop_xml_writter .= $wop_tab.$wop_tab.'<'.$node_key.'>'.htmlspecialchars( stripslashes( $node_value ) ).'</'.$node_key.'>'.$wop_br;
						}
						$wop_xml_writter .= $wop_tab.'</item>'.$wop_br;
					}

					$wop_xml_writter .= '</items>';
					echo $wop_xml_writter;
				}
			} elseif ( 'json' == $action ) {
		 ...
		 */
		public function wop_xml_2array($xml) {
	        ...
	        foreach ( $xml->children() as $r ) {
	            ...
	            if ( count( $r->children() ) == 0 ) {
	                ...
	                $arr[ $r->getName() ][] = $this->wop_xml_2array( $r );
	            }
	        }
		...
		function import($action, $filename) {
			...
			if ( 'csv' == $action ) {
				...
				$wop_xml_datas = simplexml_load_file( $_FILES[ $filename ]['tmp_name'] );
				$file_data = $this->wop_xml_2array( $wop_xml_datas );
				...
				$file_data = array();
				$wop_json_datas = wp_remote_fopen( $_FILES[ $filename ]['tmp_name'] );
				if ( false === $wop_json_datas ) {
					$wop_json_datas = file_get_contents( $_FILES[ $filename ]['tmp_name'] );
				}
				$file_datas = json_decode( $wop_json_datas, true );
			}
