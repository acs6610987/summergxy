...
if ( ! class_exists( 'wop_Core_Controller' ) ) {
	...
	class wop_Core_Controller {
	    ...
	    function __construct($objectType) {
			...
			if ( file_exists( WOP_MODEL.$this->entity.'/model.'.$this->entity.'.php' ) ) {
				$factoryObject = new FactoryModelWOP();
		...
		public function display($view, $options = array()) {
			...
			if ( ! empty( $view ) ) {
				return include( WOP_MODEL. "{$this->entity}/views/".$view );
			}
		}
		/**
		 * Return entity name.
		...
		protected function action_add_edit() {
	    	...
	    	if ( $this->entityObj->save() > 0 ) {
				...
				if ( isset( $_GET['doaction'] ) and 'edit' == $_GET['doaction'] ) {
					$response['success']  = __( $current_obj_name.' updated successfully.',WOP_TEXT_DOMAIN );
				} else { 					$response['success']  = __( $current_obj_name.' added successfully. You can manage your '.$current_obj_name.'s <a href="'.admin_url( 'admin.php?page=wop_manage_'.$this->entity ).'">here</a>.', WOP_TEXT_DOMAIN ); }
